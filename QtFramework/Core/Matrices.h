...
namespace cagd
{
    ...
    Matrix<T>::Matrix(GLuint row_count, GLuint column_count)
        : _row_count(row_count), _column_count(column_count), _data(_row_count)
    {
        for_each(
                    _data.begin(),
                    _data.end(),
                    [column_count](std::vector<T>& rowVec) { rowVec.resize(column_count); }
        );
    }
    ...
    Matrix<T>::Matrix(const Matrix& m)
        : _row_count(m._row_count), _column_count(m._column_count), _data(m._data)
    {}
    ...
    GLboolean Matrix<T>::SetRow(GLuint index, const RowMatrix<T>& row)
    {
        ...
        std::for_each(
                    _data[index].begin(),
                    _data[index].end(),
                    [&col_index, &row](T &valRef) { valRef = row[col_index++]; }
                    );
        return true;
    }
    template <typename T>
    GLboolean Matrix<T>::SetColumn(GLuint index, const ColumnMatrix<T>& column)
    {
        ...
        std::for_each(
                    _data.begin(),
                    _data.end(),
                    [&](std::vector<T> &rowVec) { rowVec[index] = column[row_index++]; }
                    );
        return true;
    }
    ...
    {
        GLuint current_count = 1;
        std::for_each(
                    _data.begin(),
                    _data.end(),
                    [&current_count](std::vector<T> &rowVec) { rowVec.resize(current_count++); }
                    );
    }
    ...
    {
        GLuint old_dim = _row_count;
        _data.resize(row_count);
        _row_count = row_count;

        for (GLuint i = old_dim; i < row_count; ++i)
        {
            _data[i].resize(i + 1);
        }
    ...
    std::istream& operator >>(std::istream& lhs, Matrix<T>& rhs)
    {
        GLuint rowCount, colCount;
        lhs >> rowCount >> colCount;

        rhs.ResizeRows(rowCount);
        rhs.ResizeColumns(colCount);
