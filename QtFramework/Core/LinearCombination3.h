...
public:
    // special constructor
    LinearCombination3(GLdouble u_min, GLdouble u_max, GLuint data_count,
    ...
    // abstract method
    //----------------
    // calculates a row matrix which consists of function values
    // {F_i(u)}_{i=0}^{data_count-1}
    virtual GLboolean
    BlendingFunctionValues(GLdouble u, RowMatrix<GLdouble> &values) const = 0;

    //----------------
    // abstract method
    //----------------
    // calculates the point and its associated (higher) order derivatives of the
    // linear combination sum_{i=0}^{data_count -1} _data[i] F_i(u) at the
    // parameter value u
    virtual GLboolean CalculateDerivatives(GLuint   max_order_of_derivatives,
                                           GLdouble u,
                                           Derivatives &d) const = 0;

    // generate image/arc
    virtual GenericCurve3 *
    GenerateImage(GLuint max_order_of_derivatives, GLuint div_point_count,
