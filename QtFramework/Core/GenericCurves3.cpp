...
GenericCurve3::GenericCurve3(GLuint maximum_order_of_derivatives,
                             GLuint point_count, GLenum usage_flag)
    : _usage_flag(usage_flag)
    , _vbo_derivative(RowMatrix<GLuint>(maximum_order_of_derivatives + 1))
    , _derivative(
          Matrix<DCoordinate3>(maximum_order_of_derivatives + 1, point_count))
{}

// special constructor
GenericCurve3::GenericCurve3(const Matrix<DCoordinate3> &derivative,
                             GLenum                      usage_flag)
    : _usage_flag(usage_flag)
...
GLboolean GenericCurve3::RenderDerivatives(GLuint order,
...
GLboolean GenericCurve3::UpdateVertexBufferObjects(GLenum usage_flag)
{
    ...
    for (GLuint d = 1; d < _derivative.GetRowCount(); ++d) {
        ...
        glBufferData(GL_ARRAY_BUFFER, higher_order_derivative_byte_size, 0,
                     _usage_flag);
...
GLboolean GenericCurve3::SetDerivative(GLuint order, GLuint index, GLdouble x,
                                       GLdouble y, GLdouble z)
{
    if (order >= _derivative.GetRowCount() ||
        index >= _derivative.GetColumnCount())
        return GL_FALSE;
...
GLboolean GenericCurve3::SetDerivative(GLuint order, GLuint index,
                                       const DCoordinate3 &d)
{
    if (order >= _derivative.GetRowCount() ||
        index >= _derivative.GetColumnCount())
        return GL_FALSE;
...
GLboolean GenericCurve3::GetDerivative(GLuint order, GLuint index, GLdouble &x,
...
GLboolean GenericCurve3::GetDerivative(GLuint order, GLuint index,
                                       DCoordinate3 &d) const
{
    if (order >= _derivative.GetRowCount() ||
        index >= _derivative.GetColumnCount())
        return GL_FALSE;
...
GLenum GenericCurve3::GetUsageFlag() const { return _usage_flag; }

// destructor
GenericCurve3::~GenericCurve3() { DeleteVertexBufferObjects(); }
